...
class TaskSerializer(serializers.ModelSerializer):
    ...
    def validate_task_list(self, value):
        if value.status == 'completed':
            ...
            if value not in user_profile.team.lists.all():
    ...
    class Meta:
        ...
        read_only_fields = ['created_at', 'updated_at' ,'status', 'created_by', 'completed_by']

class AttachmentSerializer(serializers.ModelSerializer):
    task = serializers.HyperlinkedRelatedField( queryset = Task.objects.all(), view_name='task-detail', many=False)
    class Meta:
        model = Attachment
